...
public class OptionPricerBolt extends BaseBasicBolt {
	...
	public void execute(Tuple tuple, BasicOutputCollector collector) {
		...
		logger.info("Inside execute method of OptionPricerBolt : underlyingPrice :"
				+ underlyingTickPrice);
		double optionPrice = price(data, underlyingTickPrice);
		logger.info("Final price calculated for option :"
				+ data.get("optionName").getAsString() + " is :" + optionPrice);
		logger.info("------------------------End--------------------------"
				+ System.nanoTime());
	}
	...
	private double price(JsonObject data, Double underlyingPrice) {
		...
		final Handle<BlackVolTermStructure> flatVolTS = new Handle<BlackVolTermStructure>(
				new BlackConstantVol(todaysDate, calendar, volatility,
						dayCounter));
		final Payoff payoff = new PlainVanillaPayoff(type, data.get("strike")
				.getAsDouble());
